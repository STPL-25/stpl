...
import { CheckCircle2,  XCircle,  Clock,  User,  Calendar, DollarSign, FileText, ArrowRight, Scale, Star, AlertCircle, TrendingDown, TrendingUp} from 'lucide-react';
...
const PRApprovalScreen: React.FC = () => {
  ...
  // Sample PR data with quotations
  const prData: PRData = {
    ...
    quotations: [
      ...
      {
        id: 'Q3',
        supplierName: 'Rajesh Metals & Gems',
        supplierCode: 'SUP-003',
        quotationNumber: 'QTN-2026-089',
        quotationDate: '2026-01-07',
        ...
        items: [
          {
            ...
            availability: 'In Stock',
            warranty: '12 months',
            specifications: '99.95% purity premium grade'
          },
          {
            itemId: '2',
            unitPrice: 5000,
            ...
            availability: 'In Stock',
            warranty: '2 years',
            specifications: 'Premium GIA certified, 0.5 carat'
          }
        ]
      }
    ]
  ...
  const handleSubmit = async () => {
    setLoading(true);
    // API call to approve/reject PR with selected quotation
    const payload = {
  ...
  const getLowestPrice = () => {
    if (!prData.quotations || prData.quotations.length === 0) return null;
    return Math.min(...prData.quotations.map(q => q.totalAmount));
  };
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300">
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      <Badge variant={getPriorityColor(prData.priority)}>
              ...
              <CardContent className="pt-6 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-900/50">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold text-slate-900 dark:text-slate-50">
                        ₹{prData.totalAmount.toLocaleString('en-IN')}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-900/50">
                    ...
                    <div>
                      ...
                      <p className="text-base font-semibold text-slate-900 dark:text-slate-50">
                        {new Date(prData.requestDate).toLocaleDateString('en-IN')}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-900/50">
                    ...
                    <div>
                      ...
                      <p className="text-base font-semibold text-slate-900 dark:text-slate-50">
                        {new Date(prData.requiredDate).toLocaleDateString('en-IN')}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {/* Quotations Summary */}
                {prData.quotations && prData.quotations.length > 0 && (
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-lg font-semibold flex items-center gap-2">
                        <Scale className="h-5 w-5" />
                        Supplier Quotations ({prData.quotations.length})
                      </h3>
                      <Button 
                        variant="outline" 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {prData.quotations.map((quotation) => (
                        <Card 
                          key={quotation.id} 
            ...
            <Card className="shadow-lg sticky top-6">
              ...
              <CardContent className="space-y-4">
                ...
                <Button
                  ...
                  size="lg"
                  disabled={prData.quotations && prData.quotations.length > 0 && !selectedQuotation}
                >
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <Badge variant="outline">{prData.status}</Badge>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600 dark:text-slate-400">Items</span>
                    <span className="font-semibold">{prData.items.length}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600 dark:text-slate-400">Department</span>
                    <span className="font-semibold">{prData.department}</span>
                  </div>
                  {prData.quotations && (
                    <div className="flex justify-between">
                      <span className="text-slate-600 dark:text-slate-400">Quotations</span>
                      <span className="font-semibold">{prData.quotations.length}</span>
                    </div>
                  )}
                </div>
              </CardContent>
              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  ...
                  <tbody className="divide-y">
                    ...
                    {prData.quotations?.some(q => q.discount) && (
                      <tr className="hover:bg-slate-50 dark:hover:bg-slate-900/50">
                    ...
                    {prData.quotations?.some(q => q.rating) && (
                      <tr className="hover:bg-slate-50 dark:hover:bg-slate-900/50">
                      ...
                      <table className="w-full border-collapse">
                        ...
                        <tbody className="divide-y">
                          {prData.quotations?.map((quotation) => {
                            ...
                            const isLowest = prData.quotations?.every(q => {
          ...
          <div className="space-y-4 py-4">
            {actionType === 'approve' && selectedQuotation && (
              <div className="p-3 rounded-lg bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800">
                ...
                <p className="text-sm text-green-700 dark:text-green-300">
                  {prData.quotations?.find(q => q.id === selectedQuotation)?.supplierName}
                </p>
              </div>
            )}
            ...
            <div className="p-4 rounded-lg bg-slate-50 dark:bg-slate-900 space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold">{prData.prNumber}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-600 dark:text-slate-400">Amount</span>
                <span className="font-semibold">₹{prData.totalAmount.toLocaleString('en-IN')}</span>
              </div>
            </div>
          </div>
